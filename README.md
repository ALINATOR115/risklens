...
![ΔROC-AUC](https://img.shields.io/badge/%CE%94ROC--AUC-%2B0.057%20(p%3C0.001)-brightgreen)
![Demo](https://img.shields.io/badge/Live%20Demo-offline-lightgrey)
...
Демо
Публичное демо сейчас отключено. Систему можно поднять локально за пару минут — см. Быстрый старт. В локальной версии кнопка «Демо-датасет» запускает анализ 16 920 синтетических транзакций: 80 поведенческих профилей с историей 150–200 операций на пользователя.
---
Как это работает
Конвейер из четырёх компонентов:
Маппинг колонок — определяет нужные поля во входном CSV без ручной настройки.
FeatureBuilder — считает десять поведенческих признаков по истории пользователя. Все агрегаты строятся только по предшествующим транзакциям, утечки из будущего нет.
RuleEngine — набор интерпретируемых правил с настраиваемыми порогами. Сохраняет список сработавших правил для объяснения.
RiskScorer — собирает итоговую оценку:
```
...
По `r_final` транзакция получает уровень HIGH, MEDIUM или LOW. Веса и пороги меняются в конфиге без переобучения модели.
...
Суммарная важность поведенческих признаков — 0.679 против 0.260 у базовых. SHAP-анализ на выборке из 1 000 транзакций даёт тот же порядок лидеров: `avg_amount_before` (0.082), `amount` (0.032), `diff_from_avg` (0.027). Главный сигнал для модели — не сама сумма, а то, насколько она типична для конкретного пользователя.
Ablation
